...
export default function App() {
  ...
  const saveMetadata = async () => {
    ...
    if (res.ok) {
      ...
      alert("Failed to save. Is the Docker volume mounted as read-write?");
    }
  };
        ...
        <nav className="px-4 py-2 space-y-1.5 flex-1">
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
              activeTab === 'library' ? 'bg-zinc-800/80 text-white shadow-sm border border-zinc-700/50' : 'text-zinc-400 hover:bg-zinc-800/40 hover:text-zinc-200'
            }`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center justify-between px-4 py-3 rounded-xl text-sm font-medium transition-all ${
              activeTab === 'queue' ? 'bg-zinc-800/80 text-white shadow-sm border border-zinc-700/50' : 'text-zinc-400 hover:bg-zinc-800/40 hover:text-zinc-200'
            }`}
          >
          ...
          {/* NEW TAB: Index Report */}
          <button
            onClick={() => { setActiveTab('report'); fetchReport(); }}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
              activeTab === 'report' ? 'bg-zinc-800/80 text-white shadow-sm border border-zinc-700/50' : 'text-zinc-400 hover:bg-zinc-800/40 hover:text-zinc-200'
            }`}
          >
            <FileText className="w-4 h-4 text-emerald-400" /> Catalog Report
          </button>

          <button
            onClick={() => setActiveTab('settings')}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
              activeTab === 'settings' ? 'bg-zinc-800/80 text-white shadow-sm border border-zinc-700/50' : 'text-zinc-400 hover:bg-zinc-800/40 hover:text-zinc-200'
            }`}
          >
                ...
                <div className="p-4 border-b border-zinc-800/80 bg-zinc-900/50 flex justify-between items-center">
                  ...
                  {selectedFolders.size > 0 && (
                    <button
                      ...
                      className="text-[10px] bg-indigo-500 hover:bg-indigo-400 text-white px-2 py-1.5 rounded font-bold transition-all shadow-md flex items-center gap-1"
                    >
                ...
                <div className="p-3 overflow-y-auto flex-1 space-y-1">
                  {folders.map((f) => {
                    ...
                    return (
                      <div
                        key={f.path}
                        className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm transition-all border border-transparent ${
                          selectedFolder === f.path ? 'bg-indigo-500/10 shadow-sm' : 'hover:bg-zinc-800/50'
                        }`}
                      >
                        ...
                        <div className="flex items-center gap-3 shrink-0 ml-2">
                          ...
                          <input 
                            type="checkbox" 
                            checked={isSelected} 
                            onChange={() => toggleFolderSelection(f.path)} 
                            className="w-4 h-4 rounded border-zinc-700 bg-zinc-900 text-indigo-500 focus:ring-indigo-500 cursor-pointer accent-indigo-500" 
                          />
                        </div>
                      </div>
                    );
              ...
              <div className="col-span-8 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl flex flex-col overflow-hidden backdrop-blur-xl shadow-xl">
                <div className="p-5 border-b border-zinc-800/80 bg-zinc-900/50 flex justify-between items-center h-[72px]">
                  ...
                  <div className="flex items-center gap-3">
                    {files.length > 0 && (
                      <button
                        onClick={bulkCleanTags}
                        disabled={isCleaning}
                        className="bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 disabled:opacity-50 text-xs font-bold px-4 py-2.5 rounded-lg flex items-center gap-2 transition-all"
                      >
                    ...
                    {files.length > 0 && (
                      <button
                        ...
                        disabled={isUploading}
                        className="bg-zinc-100 hover:bg-white text-zinc-900 disabled:opacity-50 text-xs font-bold px-4 py-2.5 rounded-lg flex items-center gap-2 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5"
                      >
                        ...
                        {files.map((file) => (
                          <tr key={file.path} className="group bg-zinc-950/20 hover:bg-zinc-800/40 transition-colors rounded-xl">
                            ...
                            <td className="py-3 text-zinc-400 text-xs font-medium">
                              ...
                              {file.metadata.bitrate > 0 && (
                                <div className="text-[10px] text-zinc-500 mt-0.5">{file.metadata.bitrate} kbps</div>
                              )}
                            </td>
                            <td className="py-3 pr-4 text-right rounded-r-xl">
                              <button
                                onClick={() => enqueue([file.path])}
                                className="bg-zinc-800 hover:bg-indigo-500 hover:text-white text-zinc-300 text-xs font-semibold px-3 py-1.5 rounded-md transition-all shadow-sm"
                              >
                                Upload
                              </button>
                            </td>
          ...
          {activeTab === 'report' && (
            <div className="max-w-4xl mx-auto bg-zinc-900/40 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-xl backdrop-blur-xl flex flex-col h-full">
              <div className="p-6 border-b border-zinc-800/80 bg-zinc-900/50 flex justify-between items-center">
                <div>
                  <h3 className="text-lg font-bold text-zinc-100 flex items-center gap-2">
                    <FileText className="w-5 h-5 text-emerald-400" /> Catalog Index Report
                  </h3>
                  <p className="text-xs text-zinc-500 mt-1">
                    Persistent log saved at <code className="text-indigo-400">/data/report.txt</code>
                  </p>
                </div>
                <div className="flex items-center gap-3">
                  <button
                    onClick={copyToClipboard}
                    className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs px-4 py-2.5 rounded-lg flex items-center gap-2 transition-all shadow-md"
                  >
                  ...
                  >
                    {reportSaved ? 'Saved!' : 'Save Changes'}
                  </button>
                  <button
                    onClick={clearReport}
                    className="bg-zinc-800 hover:bg-red-500/20 hover:text-red-400 border border-zinc-700 px-3 py-2.5 rounded-lg text-xs font-semibold text-zinc-400 transition-all"
                  >
                    Clear
                  </button>
                </div>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <textarea
                  className="w-full flex-1 bg-zinc-950/70 border border-zinc-800 focus:border-indigo-500 rounded-xl p-4 text-xs font-mono text-zinc-300 outline-none resize-none leading-relaxed"
                  value={reportContent}
                  onChange={(e) => setReportContent(e.target.value)}
                  placeholder="Uploaded album links will be automatically recorded here in [Year - Album](link) format..."
                />
              </div>
            </div>
          )}
          ...
          {activeTab === 'settings' && (
            <div className="max-w-2xl mx-auto bg-zinc-900/40 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-xl backdrop-blur-xl">
              ...
              <div className="p-8 space-y-6">
                <div>
                  <label className="flex text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2">Bot Token</label>
                  <input
                    type="password"
                    className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-indigo-500 rounded-xl px-4 py-3 text-sm text-zinc-100 outline-none font-mono"
                    value={settings.bot_token}
                    onChange={(e) => setSettings({ ...settings, bot_token: e.target.value })}
                  />
                </div>
                <div>
                  <label className="flex text-xs font-bold uppercase tracking-wider text-zinc-500 mb-2">Default Destination</label>
                  <input
                    type="text"
                    className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-indigo-500 rounded-xl px-4 py-3 text-sm text-zinc-100 outline-none font-mono"
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      type="number"
                      className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-indigo-500 rounded-xl px-4 py-3 text-sm text-zinc-100 outline-none"
              ...
              <div className="p-6 bg-zinc-900/80 border-t border-zinc-800/80 flex items-center justify-between">
                ...
                {botStatus && (
                  <div className={`text-xs font-semibold px-4 py-2 rounded-xl border ${
                    botStatus.includes('Error') ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                  }`}>
                    {botStatus}
                  </div>
                )}
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 max-w-md w-full flex flex-col shadow-2xl relative">
                <div className="flex justify-between items-center mb-6">
                  ...
                  <button onClick={() => setEditingFile(null)} className="p-2 bg-zinc-800 hover:bg-zinc-700 rounded-lg transition-all text-zinc-400">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="text-xs font-bold uppercase text-zinc-500">Track #</label>
                      <input
                        type="text"
                        className="w-full bg-zinc-950 border border-zinc-800 rounded-lg px-3 py-2 text-sm text-zinc-100 outline-none focus:border-indigo-500"
                ...
                <button
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 rounded-xl mt-6 transition-all shadow-lg"
                >
                  Save Changes
                </button>
              </div>
