...
export default function App() {
  ...
  const testBot = async () => {
    ...
    const data = await res.json();
    if (res.ok && data.bot_username) { setBotStatus(`Connected: @${data.bot_username}`); } else { setBotStatus(`Error: ${data.detail || 'Connection failed'}`); }
  };
  ...
  const saveMetadata = async () => {
    const res = await fetch('/api/library/metadata', {
      ...
      body: JSON.stringify({ filepath: editingFile.path, ...editForm })
    });
    if (res.ok) {
      setEditingFile(null);
      loadFolderFiles(selectedFolder);
    } else { alert("Failed to save. Is the Docker volume mounted as read-write?"); }
  };

  const bulkCleanTags = async () => {
    if(!window.confirm("This will permanently delete lyrics, comments, and non-essential ID3 tags from all MP3s in this folder. Proceed?")) return;
    setIsCleaning(true);
    const res = await fetch('/api/library/metadata/clean', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ folder_path: selectedFolder })
    });
    const data = await res.json();
    setIsCleaning(false);
    if(res.ok) { alert(`Successfully cleaned extra tags from ${data.cleaned} files!`); }
  };
              ...
              <div className="col-span-8 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl flex flex-col overflow-hidden backdrop-blur-xl shadow-xl">
                <div className="p-5 border-b border-zinc-800/80 bg-zinc-900/50 flex justify-between items-center h-[72px]">
                  ...
                  <div className="flex items-center gap-3">
                    {files.length > 0 && (
                      <button onClick={bulkCleanTags} disabled={isCleaning} className="bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 disabled:opacity-50 text-xs font-bold px-4 py-2.5 rounded-lg flex items-center gap-2 transition-all">
                        <Trash2 className="w-4 h-4" /> Clean Tags
                      </button>
                    )}
          ...
          {activeTab === 'settings' && (
             <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
               <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-xl backdrop-blur-xl h-fit">
                 <div className="p-5 border-b border-zinc-800/80 bg-zinc-900/50"><h3 className="text-base font-bold text-zinc-100 flex items-center gap-2"><SettingsIcon className="w-4 h-4 text-indigo-400" /> API Configuration</h3></div>
                 <div className="p-6 space-y-5">
                   <div><label className="flex text-[11px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Bot Token</label><input type="password" className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-indigo-500 rounded-lg px-3 py-2 text-sm text-zinc-100 outline-none font-mono" value={settings.bot_token} onChange={(e) => setSettings({ ...settings, bot_token: e.target.value })} /></div>
                   <div><label className="flex text-[11px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Default Destination</label><input type="text" className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-indigo-500 rounded-lg px-3 py-2 text-sm text-zinc-100 outline-none font-mono" value={settings.default_dest} onChange={(e) => setSettings({ ...settings, default_dest: e.target.value })} /></div>
                   <div className="grid grid-cols-2 gap-4">
                     <div><label className="flex text-[11px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Min Delay (s)</label><input type="number" className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-indigo-500 rounded-lg px-3 py-2 text-sm text-zinc-100 outline-none" value={settings.delay_per_file_min} onChange={(e) => setSettings({ ...settings, delay_per_file_min: parseInt(e.target.value) || 0 })} /></div>
                     <div><label className="flex text-[11px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Max Delay (s)</label><input type="number" className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-indigo-500 rounded-lg px-3 py-2 text-sm text-zinc-100 outline-none" value={settings.delay_per_file_max} onChange={(e) => setSettings({ ...settings, delay_per_file_max: parseInt(e.target.value) || 0 })} /></div>
                   </div>
                 </div>
               </div>
               ...
               <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl overflow-hidden shadow-xl backdrop-blur-xl h-fit flex flex-col">
                 <div className="p-5 border-b border-zinc-800/80 bg-zinc-900/50 flex justify-between items-center"><h3 className="text-base font-bold text-zinc-100 flex items-center gap-2"><FileText className="w-4 h-4 text-emerald-400" /> Index Post Links</h3></div>
                 <div className="p-6 space-y-5 flex-1 flex flex-col">
                   <div><label className="flex text-[11px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Report Channel (e.g. -10012345)</label><input type="text" className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-emerald-500 rounded-lg px-3 py-2 text-sm text-zinc-100 outline-none font-mono" value={settings.report_channel} onChange={(e) => setSettings({ ...settings, report_channel: e.target.value })} /></div>
                   <div><label className="flex text-[11px] font-bold uppercase tracking-wider text-zinc-500 mb-2">Target Message ID (e.g. 42)</label><input type="text" className="w-full bg-zinc-950/50 border border-zinc-800 focus:border-emerald-500 rounded-lg px-3 py-2 text-sm text-zinc-100 outline-none font-mono" value={settings.report_message_id} onChange={(e) => setSettings({ ...settings, report_message_id: e.target.value })} /></div>
                   <div className="flex-1 flex flex-col min-h-[120px]">
                     <div className="flex justify-between items-center mb-2"><label className="flex text-[11px] font-bold uppercase tracking-wider text-zinc-500">Accumulated Markdown HTML</label><button onClick={() => setSettings({...settings, report_text: ''})} className="text-[10px] text-red-400 hover:text-red-300 hover:underline">Clear Text</button></div>
                     <textarea className="w-full flex-1 bg-zinc-950/50 border border-zinc-800 focus:border-emerald-500 rounded-lg px-3 py-2 text-xs text-zinc-400 outline-none font-mono resize-none" value={settings.report_text} onChange={(e) => setSettings({ ...settings, report_text: e.target.value })} placeholder="Links will automatically generate here..." />
                   </div>
                 </div>
               </div>
               ...
               <div className="col-span-1 md:col-span-2 p-6 bg-zinc-900/80 border border-zinc-800/80 rounded-2xl flex items-center justify-between">
                 <button onClick={testBot} className="bg-indigo-600 hover:bg-indigo-500 text-sm font-semibold px-8 py-3 rounded-xl text-white transition-all shadow-lg shadow-indigo-500/25">Save All Configurations</button>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 max-w-md w-full flex flex-col shadow-2xl relative">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-xs text-zinc-400 mt-1 truncate max-w-[250px]">{editingFile.filename}</p>
                  </div>
